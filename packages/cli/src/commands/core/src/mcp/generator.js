...
export class MCPServerGenerator {
     ...
     */
    static generateServerFile(options) {
        return `import { createMCPServer, PromptOrDieMCPServer } from 'prompt-or-die/mcp';
import { createTransport } from 'prompt-or-die/mcp/transports';
...
`;
     ...
     */
    static generateConfigFile(options) {
        const serverConfig = {
            name: options.name,
            version: '1.0.0',
            description: options.description || `MCP Server for ${options.name}`,
        ...
        return `import { MCPServerConfig } from 'prompt-or-die/mcp';
...
`;
     ...
     */
    static generateStorageFile(options) {
        let imports = `import { PromptStorage, MemoryPromptStorage, FilePromptStorage } from 'prompt-or-die/mcp';`;
        let createFunction = '';
        if (options.storage === 'database') {
...
`;
     ...
     */
    static generateDeploymentInstructions(options) {
...
\`\`\``;
     ...
     */
    static generateEnvExample(options) {
...
`;
     ...
     */
    static generateBuildScript(options) {
...
`;
     ...
     */
    static generateStartScript(options) {
        return `#!/usr/bin/env node
